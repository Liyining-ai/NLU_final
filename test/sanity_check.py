...
def test_gpt2(model_size='gpt2',model_path=None):
  ...
  if model_path:
      openai_model = OpenAIGPT2Model.from_pretrained(model_path)  # Load from local path
      gpt = GPT2Model.from_pretrained(model_path, **model_size_to_params(model_size))  # Load from local path
  else:
      openai_model = OpenAIGPT2Model.from_pretrained(model_size)  # Default to loading from Hugging Face
...
if __name__ == '__main__':
  model_path = "/home/ubuntu/NLP/ckpts"
  test_gpt2('gpt2', model_path=model_path)
