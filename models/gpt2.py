...
class GPT2Model(GPTPreTrainedModel):
  ...
  @classmethod
  def from_pretrained_only_wordembed(cls, model='gpt2', d=768, l=12, num_heads=12):
      """
      Initialize model from scratch with only pretrained word embeddings loaded.
      All transformer weights are randomly initialized.
      """
      # Load pretrained model only to extract word embeddings
      gpt_model = OpenAIGPT2Model.from_pretrained(model).eval()
      ...
      # Only load word embeddings from pretrained model
      our_model.word_embedding.load_state_dict(gpt_model.wte.state_dict())
      print(f"Initialized model from scratch with only word embeddings loaded.")
      print(f"Architecture: hidden_size={d}, num_layers={l}, num_heads={num_heads}")
